...
void LcdSetCursor(uint8_t status)
{
	if(status == LCD_CURSOR_OFF){
		LcdWriteCmd(0x0C);
	}else if (status == LCD_CURSOR_ON){
		LcdWriteCmd(0x0F);
	}
	HAL_Delay(1);
}
...
static void LcdWriteNibble(uint8_t lcdData)
{
	HAL_GPIO_WritePin(LCD_DATA_PORT, DB7, lcdData & 0x80);
	HAL_GPIO_WritePin(LCD_DATA_PORT, DB6, lcdData & 0x40);
	HAL_GPIO_WritePin(LCD_DATA_PORT, DB5, lcdData & 0x20);
	HAL_GPIO_WritePin(LCD_DATA_PORT, DB4, lcdData & 0x10);
